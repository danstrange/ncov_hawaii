...
## filter to remove nextclade bad sequences, sequences with >=1200 Ns, and vadr-failed sequences

cut -f2,10,15 nextclade_report.$datestamp.tsv | tail -n+2 | awk '($3 < 1200)&&(/good/||/mediocre/)' | cut -f1 > qc_pass.$datestamp.list

mv sequences_$datestamp.fa sequences_$datestamp.all.fa
cdbfasta sequences_$datestamp.all.fa
#cut -f2,15,44 nextclade_report.$datestamp.tsv |tail -n+2 |python -c "import sys;[sys.stdout.write(line.split()[0]+'\n') for line in sys.stdin if len(line.split())>1 and int(line.split()[1])<1200]" |cdbyank sequences_$datestamp.all.fa.cidx>sequences_$datestamp.fa
...
#(head -1 metadata_$datestamp.all.txt; cat metadata_$datestamp.all.txt|grep $(cut -f2,15,44 nextclade_report.$datestamp.tsv |tail -n+2 |python -c "import sys;[sys.stdout.write(line.split()[0]+'\\|') for line in sys.stdin if len(line.split())>1 and int(line.split()[1])<1200]" |sed 's/\\|$//'))> metadata_$datestamp.txt
